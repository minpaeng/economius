        ...
        <>
            ...
            {distButtons.map(value => (
                <div key={value} style={{ position: 'absolute', top: '5vh', left: `${30 + value * 5}vw` }}>
                    <button style={{ backgroundColor: isMoving ? 'orange' : 'white' }} onClick={() => (setIsMoving(true), setMoveDist(value))}>
                        {value}
                    </button>
                </div>
            ))}
            ...
            {playerButtons.map((nickname, index) => (
                <div key={index} style={{ position: 'absolute', top: '10vh', left: `${35 + index * 5}vw` }}>
                    <button style={{ backgroundColor: index === nowPlayer ? 'orange' : 'white' }} onClick={() => setNowPlayer(index)}>
                        {nickname}
                    </button>
                </div>
            ))}
