...
function BigEventRound() {
    ...
    function Rate_Gold_EstateList(InterestRateChange, GoldChange, BuildingChange) {
        const DataArr = [];
        if (issue) {
            DataArr.push({
    ...
    const Rate_Gold_Estate = Rate_Gold_EstateList(issue?.interestRateChange, issue?.goldChange, issue?.buildingChange);
    ...
    return (
        <Modal isOpen={isModalOpen} style={modalStyle} onRequestClose={closeModal}>
            <S.BigEvent>
                <S.BigEventLeft />
                <S.BigEventRight style={{ color: issue?.type === 'BOOM' ? 'black' : 'white' }}>
                    ...
                    <div>
                        ...
                        <S.BigEventRightRate_Gold_Estate>
                            {Rate_Gold_Estate.map((item, idx) => {
                                return <BigEventRoundItem title={item.title} changeRate={item.changeRate} key={idx} />;
                            })}
                        </S.BigEventRightRate_Gold_Estate>
                    </div>
