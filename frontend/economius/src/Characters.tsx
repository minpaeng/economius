...
function Characters({ CharacterArr }) {
    ...
    // 플레이어의 닉네임, 캐릭터(idx)
    const [player1, setPlayer1] = useState<[string, number]>(['P1', 4]);
    const [player2, setPlayer2] = useState<[string, number]>(['P2', 9]);
    const [player3, setPlayer3] = useState<[string, number]>(['P3', 1]);
    const [player4, setPlayer4] = useState<[string, number]>(['P4', 6]);
