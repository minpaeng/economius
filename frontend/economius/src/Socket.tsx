...
function PlayerSocket() {
  ...
  const [nowPlayerPosition, setNowPlayerPosition] = useRecoilState(
    NowPlayerPositionState
  );
  const [movementCard, setMovementCard] = useRecoilState(MovementCardState);
  const [movementCardOpen, setMovementCardOpen] = useRecoilState(
    MovementCardOpenState
  );
  const [monthlyModalOpen, setMonthlyModalOpen] = useRecoilState(
    MonthlyModalOpenState
  );
  ...
  const [realEstateInfo, setRealEstateInfo] =
    useRecoilState(RealEstateInfoState);
  ...
  // 자산별 거래 여부
  const [tradeRealEstate, setTradeRealEstate] =
    useRecoilState(TradeRealEstateState);
  ...
  const [tradeInsurance, setTradeInsurance] =
    useRecoilState(TradeInsuranceState);
  const [tradeInsuranceConfirm, setTradeInsuranceConfirm] = useRecoilState(
    TradeInsuranceConfirmState
  ); // 보험 확인 버튼
  ...
  const [roomJoinUsersNickname, setRoomJoinUsersNickname] = useRecoilState(
    RoomJoinUsersNicknameState
  );
  ...
  const [roomJoinUsersId, setRoomJoinUsersId] =
    useRecoilState(RoomJoinUsersIdState);
    ...
    // 게임 시작 pub에 대한 결과를 반환 받으면
      ...
      connect().then(function () {
        stompClient.current.send(
          `/pub/${roomId}/viewMovementCard`,
          {},
          JSON.stringify({ player: playerToRoll })
        );
      });
      // }
    }
    ...
    connect().then(function () {
      ...
      if (roomId != 0 && stompClient.current.connected) {
        ...
        stompClient.current.subscribe(
          `/sub/player/${playername}`,
          personalCallBackFunction
        );
        stompClient.current.subscribe(
          `/sub/${roomId}`,
  ...
  async function connect() {
    ...
    if (!stompClient.current || !stompClient.current.connected) {
      ...
      stompClient.current = Stomp.over(
        () => new sockjs("https://j9b109.p.ssafy.io/ws")
      );
  ...
  useEffect(() => {
    ...
    // 금거래소 방문
      ...
      connect().then(function () {
        stompClient.current.send(
          `/pub/${roomId}/selectGolds`,
          {},
          JSON.stringify({ player: nowPlayer + 1 })
        );
      });
    }
    // 은행 방문
    ...
    if (tradeInsurance[0]) {
      connect().then(function () {
        stompClient.current.send(
          ...
          {},
          JSON.stringify({ player: nowPlayer + 1, insuranceId: 3 })
        );
      });
    } else {
      connect().then(function () {
    ...
    if (tradeInsurance[1]) {
      connect().then(function () {
        stompClient.current.send(
          `/pub/${roomId}/joinInsurance`,
          {},
          JSON.stringify({ player: nowPlayer + 1, insuranceId: 4 })
        );
      });
    } else {
      connect().then(function () {
    ...
    if (tradeInsurance[2]) {
      connect().then(function () {
        stompClient.current.send(
          `/pub/${roomId}/joinInsurance`,
          {},
          JSON.stringify({ player: nowPlayer + 1, insuranceId: 1 })
        );
      });
    } else {
      connect().then(function () {
    ...
    if (tradeInsurance[3]) {
      connect().then(function () {
        stompClient.current.send(
          `/pub/${roomId}/joinInsurance`,
          {},
          JSON.stringify({ player: nowPlayer + 1, insuranceId: 2 })
        );
      });
    } else {
      connect().then(function () {
