...
function PlayerSocket() {
  ...
  useEffect(() => {
    ...
    stompClient.current = Stomp.over(
      () => new sockjs("http://j9b109.p.ssafy.io:8080/ws")
    );
    ...
    // 보험 방문
      ...
      if (stompClient.current) {
        stompClient.current.send(
          ...
          {},
          JSON.stringify({ player: nowPlayer + 1, insuranceId: "" })
        );
      }
    }
    // 찬스 카드 방문
    ...
    if (tradeGold[0]) {
      if (stompClient.current) {
        stompClient.current.send(
          "/pub/1/buyGolds",
          {},
          JSON.stringify({ player: nowPlayer + 1, goldAmount: buyAmount })
        );
      ...
      if (stompClient.current) {
        stompClient.current.send(
          "/pub/1/sellGolds",
          {},
          JSON.stringify({ player: nowPlayer + 1, goldAmount: sellAmount })
        );
        setTradeGold([false, false]);
      }
    }
